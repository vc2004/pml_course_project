...
This report focus on data from Weight Lifting Exercise. Among the 5 classes of weight lifting (Class A,B,C,D,E), only Class A is the correct training method, while others are training method with common mistakes. The report use other varaibles in the dataset to predict the class of weight lifting.
...
The random forest is the tried as the first algorithm. The result is shown as below, the error rate on training data is 99.7% and the Out of Sample Error is 0.3%.
...
Since the Random Forest has already generate a very good result and fitted model, and it has proven performance in the cross validation, it will be used in the prediction. Transform the test set into the same data format as the training set to fit the prediction model. 
